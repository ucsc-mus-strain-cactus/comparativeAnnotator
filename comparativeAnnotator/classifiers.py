...
from comparativeAnnotator.annotation_utils import *
    ...
    def run(self):
        for ens_id, a in self.annotation_iterator():
            ...
            if a.strand is True and a_frames[0] != 0 or a.strand is False and a_frames[-1] != 0:
                ...
                self.details_dict[ens_id].append(cds_coordinate_to_bed(a, 0, 3, self.rgb, self.column))
            else:
                self.classify_dict[ens_id] = 0
    ...
    def run(self):
        for ens_id, a in self.annotation_iterator():
            ...
            if short_cds(a):
                ...
                continue
            if a.cds_size % 3 != 0:
                bed_rec = chromosome_coordinate_to_bed(a, a.thick_start, a.thick_stop, self.rgb, self.column)
    ...
    def run(self):
        ...
        for ens_id, a in self.annotation_iterator():
            ...
            if short_cds(a):
                ...
                bed_rec = cds_coordinate_to_bed(a, 0, 3, self.rgb, self.column)
                self.details_dict[ens_id].append(bed_rec)
                self.classify_dict[ens_id] = 1
            else:
                self.classify_dict[ens_id] = 0
    ...
    def run(self):
        ...
        for ens_id, a in self.annotation_iterator():
            ...
            if short_cds(a):
                ...
                bed_rec = cds_coordinate_to_bed(a, a.cds_size - 3, a.cds_size, self.rgb, self.column)
                self.details_dict[ens_id].append(bed_rec)
                self.classify_dict[ens_id] = 1
            else:
                self.classify_dict[ens_id] = 0
    ...
    def run(self, cds_filter_fn=is_cds, mult3=False, skip_n=True):
        ...
        for ens_id, a in self.annotation_iterator():
            for intron in a.intron_intervals:
                is_gap = analyze_intron_gap(a, intron, self.ref_seq_dict, cds_filter_fn, skip_n, mult3)
                if is_gap is True:
                    bed_rec = interval_to_bed(a, intron, self.rgb, self.column)
                    self.details_dict[ens_id].append(bed_rec)
...
class CdsNonCanonSplice(AbstractClassifier):
    ...
    def run(self, cds_filter_fn=is_cds, splice_dict={"GT": "AG"}):
        ...
        for ens_id, a in self.annotation_iterator():
            for intron in a.intron_intervals:
                splice_is_good = analyze_splice(intron, a, self.ref_seq_dict, cds_filter_fn, splice_dict)
                if splice_is_good is True:
                    bed_rec = splice_intron_interval_to_bed(a, intron, self.rgb, self.column)
                    self.details_dict[ens_id].append(bed_rec)
            self.classify_dict[ens_id] = len(self.details_dict[ens_id])
        self.dump_results_to_disk()
    ...
    def run(self):
        ...
        for ens_id, a in self.annotation_iterator():
            ...
            for i, codon in read_codons_with_position(cds, offset, skip_last=True):
                ...
                if amino_acid == "*":
                    bed_rec = cds_coordinate_to_bed(a, i, i + 3, self.rgb, self.column)
                    self.details_dict[ens_id].append(bed_rec)
            self.classify_dict[ens_id] = len(self.details_dict[ens_id])
        self.dump_results_to_disk()
    ...
    def run(self):
        for ens_id, a in self.annotation_iterator():
            if short_cds(a) is True and a.cds_size != 0:
                bed_rec = cds_coordinate_to_bed(a, 0, a.cds_size, self.rgb, self.column)
                self.details_dict[ens_id].append(bed_rec)
                self.classify_dict[ens_id] = 1
            else:
                self.classify_dict[ens_id] = 0
