...
id_template = 'MGP_{strain_id:.9}{strain_suffix}_{tag_type}{unique_id:07d}'
...
def parse_args():
    ...
    parser.add_argument('out_table', help='Output data table', type=argparse.FileType('w'))
...
def main():
    ...
    args.out_table.write('\t'.join(['#transcript_id', 'parent_transcript_id', 'gene_id', 'parent_gene_id',
                                    'gene_biotype', 'transcript_biotype']) + '\n')
    for rec in tx_table.itervalues():
        args.out_table.write('\t'.join(rec) + '\n')
