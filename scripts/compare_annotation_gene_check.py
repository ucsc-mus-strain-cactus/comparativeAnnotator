...
fields = (("UtrGap", "utrGap"), ("CdsGap", "cdsGap"), ("BeginStart", "noStart"), ("EndStop", "noStop"), 
          ("UnknownCdsSplice", "unknownCdsSplice"), )
...
def main():
    ...
    for my_field, mark_field in fields:
        with open(os.path.join(d, "mark_diff_my_" + my_field + ".bed"), "w") as outf:
            ...
            my_this = {tuple(x.split()[:3]) for x in my_records if my_field in x}
            mark_this = {tuple(x.split()[:3]) for x in mark_records if mark_field in x}
